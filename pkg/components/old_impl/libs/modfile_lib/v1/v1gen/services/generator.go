...
func GenServices(mfSs map[string]model.Service) (map[string]*module_lib2.Service, error) {
	mSs := make(map[string]*module_lib2.Service)
...
func GenAuxServices(mfSs map[string]model.AuxService) (map[string]*module_lib2.AuxService, error) {
	mAs := make(map[string]*module_lib2.AuxService)
	for ref, mfS := range mfSs {
		mBMs, err := GenBindMounts(mfS.Include)
		if err != nil {
		...
		mAs[ref] = &module_lib2.AuxService{
			Name:       mfS.Name,
			RunConfig:  GenRunConfig(mfS.RunConfig),
			BindMounts: mBMs,
			Tmpfs:      mTMs,
...
func GenRunConfig(mfRC model.RunConfig) module_lib2.RunConfig {
	mRC := module_lib2.RunConfig{
		MaxRetries:  5,
...
func GenBindMounts(mfBMs []model.BindMount) (map[string]module_lib2.BindMount, error) {
	mBMs := make(map[string]module_lib2.BindMount)
	for _, mfBM := range mfBMs {
		...
		mBMs[mfBM.MountPoint] = module_lib2.BindMount{
			Source:   mfBM.Source,
...
func GenTmpfsMounts(mfTMs []model.TmpfsMount) (map[string]module_lib2.TmpfsMount, error) {
	mTMs := make(map[string]module_lib2.TmpfsMount)
...
func GenHttpEndpoints(mfHEs []model.HttpEndpoint) (map[string]module_lib2.HttpEndpoint, error) {
	mHEs := make(map[string]module_lib2.HttpEndpoint)
	for _, mfHE := range mfHEs {
		if _, ok := mHEs[mfHE.ExtPath]; ok {
			return nil, fmt.Errorf("duplicate '%s'", mfHE.ExtPath)
		}
		mHE := module_lib2.HttpEndpoint{
			Name: mfHE.Name,
			Port: mfHE.Port,
			Path: mfHE.Path,
			ProxyConf: module_lib2.HttpEndpointProxyConf{
				Headers:   mfHE.ProxyConf.Headers,
				WebSocket: mfHE.ProxyConf.WebSocket,
			},
			StringSub: module_lib2.HttpEndpointStrSub{
...
func GenPorts(mfSPs []model.SrvPort) ([]module_lib2.Port, error) {
	var mPs []module_lib2.Port
	for _, mfSP := range mfSPs {
		proto := module_lib2.TcpPort
		...
		if lep == 1 {
			mPs = append(mPs, module_lib2.Port{
				Name:     mfSP.Name,
				Number:   ep[0],
				Protocol: proto,
				Bindings: hp,
